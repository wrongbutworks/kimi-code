...
在较长的提示词中，也可以在空白字符后（包括后续行的行首）输入 `/` 打开仅包含 Skill 的补全菜单。这样可以在一条提示词里引用多个 Skill：Kimi Code 会将它们一起激活，与提示词作为同一轮次运行（一次 `/undo` 即可整体撤销），提示词原文保持不变。内置命令和 plugin 命令仍需放在输入开头。
